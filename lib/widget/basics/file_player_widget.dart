...
class _FilePlayerWidgetState extends State<FilePlayerWidget> {
  ...
  Widget build(BuildContext context) => Scaffold(
        body: Column(
          children: [
            ...
            buildAddButton(),//button for choosing the file
          ],
        ),
      );
  ...
  Future<File> pickVideoFile() async {
    final result = await FilePicker.platform.pickFiles(type: FileType.video);//We pick a file of type video
    if (result == null) return null;

    return File(result.files.single.path);//We get the path and convert it into a File and return
  }
}
